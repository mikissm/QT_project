...
void MainWindow::startNewGame()
{
    ...
    QPointF safe = findSafeSpawn(playerRadius); // 안전한 위치 찾기
    teleportPlayerTo(safe, /*broadcast=*/true); // 위치 이동
    startMyInvulnerability(2000);  // 무적 상태로 설정

    // 맵 리셋(호스트만 새 시드 생성/배포)
    if (isHost) {
        mapSeed = QRandomGenerator::global()->generate();  // 새 맵 시드 생성
        buildRandomMap(mapSeed, mapW, mapH);  // 새 맵 생성
        setupMiniMap();  // 미니맵 갱신
