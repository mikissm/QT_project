                  ...
                  QObject* ctx)
{
    ...
    case MULTI: // 따발총: 여러 발 퍼짐
        spawnSpread_(scene, playerPos, targetScenePos,
                     3, 15.0, bulletSpeed_, rangePx_, 6, ctx);
        break;
    case POWER: // 파워총: 크고 빠른 1발
        spawnOneBullet_(scene, playerPos, targetScenePos,
                        bulletSpeed_*1.5, rangePx_, 14, ctx);
        break;
    case HOMING: // 유도총: 방향 계속 갱신
        spawnOneBullet_(scene, playerPos, targetScenePos,
                        bulletSpeed_, rangePx_, 10, ctx);
        //  실제 유도는 QTimer 안에서 목표 좌표를 따라가도록 갱신하면 됨
        break;
    case SHOTGUN: // 샷건: 여러 발 랜덤 퍼짐
        spawnSpread_(scene, playerPos, targetScenePos,
                     6, 40.0, bulletSpeed_, rangePx_, 5, ctx);
        break;
    }
    return true;
}
