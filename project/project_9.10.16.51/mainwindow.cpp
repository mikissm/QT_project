...
void MainWindow::spawnBullet(const QString& kindOrName, const QPointF& start, const QPointF& target, bool fromPeer)
{
    ...
    auto radiusForKind = [&](const QString& k)->qreal {
        if (k == "sniper")  return 7.0;  // 저격: 큼
        if (k == "multi")   return 3.0;  // 따발: 보통
        if (k == "shotgun") return 2.5;  // 샷건: 작게
        return 3.0;
    };
...
{
    // 호스트가 아닐 경우에는 게임 오버 판정을 하지 않음
    if (!isHost) return;

    // 이미 게임오버 처리 중이면 무시
    if (inGameOver) return;

    // 상대가 아직 활성화되지 않았다면 peerHP는 무시(혼자 놀기 허용)
    const bool peerDeadOk = peerActive && (peerHP <= 0);
    if (myHP <= 0 || peerDeadOk) {
        inGameOver = true;  // ★ 재진입 금지

        QMessageBox::information(this, "게임 종료", "게임 오버! 재시작 준비");
...
void MainWindow::startNewGame()
{
    ...
    currentWeaponPtr = basicWeapon.get();
    if (currentWeaponPtr) {
        currentWeaponPtr->setAmmo(currentWeaponPtr->magSize());
    }
    updateWeaponUi();

    // 랜덤 안전 위치로 텔레포트 + 잠깐 무적
    QPointF safe = findSafeSpawn(playerRadius); // 첫 번째 플레이어의 위치
    teleportPlayerTo(safe, /*broadcast=*/true);
    startMyInvulnerability(2000);

    // 참가자의 위치도 랜덤하게 설정
    if (!isHost) {
        safe = findSafeSpawn(peerRadius);  // 두 번째 플레이어(참가자)의 위치
        teleportPlayerTo(safe, /*broadcast=*/false); // 참가자는 브로드캐스트하지 않음
    }

    // 맵 리셋(호스트만 새 시드 생성/배포)
    if (isHost) {
        mapSeed = QRandomGenerator::global()->generate();
        buildRandomMap(mapSeed, mapW, mapH);
        setupMiniMap();
...
void MainWindow::udpDataReceived() {
    while (udp && udp->hasPendingDatagrams()) {
        ...
        if (type=="map_seed") {
            ...
            if (dmg > 0 && !inGameOver) {                        // ★ 게임오버 중이면 무시
            ...
            // 참가자 들어오면 게임 시작 여부 묻기 (양쪽 다 묻게 함)
            ...
            restartReadyPeer = true;
            if (restartReadyLocal && restartReadyPeer && !gameStarted) {
                startNewGame();
            }
        }
    }
...
QPointF MainWindow::findSafeSpawn(qreal radius, int tries) const
{
    ...
    // 여러 번 시도 후에도 두 번째 플레이어의 안전한 위치를 못 찾은 경우, 랜덤 위치로 선택
    const int grid = 20;
    ...
    return R.center();  // 안전한 위치를 못 찾으면 중앙으로 반환
}
