...
private:
    ...
    // 게임 상태 플래그
    bool gameStarted       = false;  // 실제 게임 진행 중 여부
    bool inGameOver        = false;  // 게임오버 팝업/로직 중복 방지
    bool restartReadyLocal = false;  // 내가 '시작/재시작' 준비 완료
    bool restartReadyPeer  = false;  // 상대가 '시작/재시작' 준비 완료
    bool peerActive        = false;  // 상대 접속/위치 수신 등으로 활성화됐는가
