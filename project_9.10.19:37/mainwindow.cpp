...
void MainWindow::checkGameOver()
{
    ...
    if (!myDead && !peerDead) return; // 아직 종료 아님
...
void MainWindow::startNewGame()
{
    ...
    if (isHost) {
        ...
        // ★ 3) 맵시드/HP 동기화 전송 (텔레포트 후 전송해도 무방)
        if (udp && !peerAddr.isNull() && peerPort) {
        ...
        // ★ 참가자는 'map_seed' 수신 후 buildRandomMap()이 끝난 다음에
        //    udpDataReceived(map_seed)에서 findSafeSpawn() + teleport를 수행함.
        //    (지금은 텔레포트 금지)
        mapReady = false;                // 움직임 업데이트에서 안전 처리
        peerItem->setVisible(false);
        appendChatMessage("시스템","맵 시드 수신 대기 중...");
    }
...
void MainWindow::udpDataReceived() {
    while (udp && udp->hasPendingDatagrams()) {
        ...
        if (type=="map_seed") {
            ...
            // 참가자만 신뢰해서 적용 (호스트가 자기 자신에게 적용할 필요 없음)
